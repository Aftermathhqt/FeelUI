--[[
## Animation for Gain/Loss from ls:UI
## https://www.wowinterface.com/downloads/info22662-lsUI.html

## PRE UPDATE FUNCTIONS

local function UpdateGainLossPoints(self)
	self.GainLossIndicators:UpdatePoints("HORIZONTAL" / "VERTICAL")
end

local function UpdateGainLossThreshold(self)
	self.GainLossIndicators:UpdateThreshold(0.01)
end

local function UpdateGainLossColors(self)
	self.GainLossIndicators:UpdateColors()
end

self.GainLossIndicators = A:CreateGainLossIndicators(StatusBar)
self.UpdateGainLossColors = UpdateGainLossColors
self.UpdateGainLossPoints = UpdateGainLossPoints
self.UpdateGainLossThreshold = UpdateGainLossThreshold

## UPDATE FUNCTIONS

self.GainLossIndicators:Update(min, max)

self:UpdateGainLossColors()
self:UpdateGainLossPoints()
self:UpdateGainLossThreshold()
--]]

local UI, Settings, Media, Language = select(2, ...):Call()
...
function UI:CreateGainLossIndicators(object)
	local Texture = [[Interface\AddOns\ClassicUI\Media\Textures\Melli.tga]]
